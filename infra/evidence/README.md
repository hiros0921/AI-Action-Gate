...
> **【重要】公開する前に、2つを伏せてください。**
>
> | 伏せるもの | どこに写るか | なぜ |
> |---|---|---|
> | **Function URL** | `aws-run.txt`・スクリーンショット | 認証タイプが `NONE`。動いているあいだは誰でも叩ける |
> | **AWSアカウントID（12桁）** | `iam-simulate-audit.json`・`teardown.txt` の ARN | 秘密ではないが、公開リポジトリに置く必要もない |
>
> **コミットしてしまうと、あとから消しても履歴には残ります。**
> 伏せる手順は `../DEPLOY.md` の「証跡を伏せる（git add より前に）」にあります。
